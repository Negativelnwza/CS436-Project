// script.js - Scientific Calculator Mode (with Shunting-Yard for precedence)
(function(){
      ...
      expr += '.';
        ...
        if(last === '' || isBinaryOperator(last) || isLParen(last) || isFunction(last)) {
            expr += token;
             ...
             // If preceding a number or ), insert implicit multiplication (for robust calculator)
             // For simplicity, we just block it for now
             return; 
        }
